...
#     try:
#         # Hlavní smyčka pro příjem a zpracování zpráv
#         while True:
#             # Přijmout zprávu jako JSON objekt
...
#             optimization_prompt = QUERY_OPTIMIZATION_PROMPT + user_message
#             try:
#                 optimized_query = await call_openrouter_chat(
#                     optimization_prompt,
#                     model_override="google/gemini-2.5-flash-lite-preview-09-2025",  # Rychlý a levný model
#                     max_tokens=256,  # Krátká odpověď stačí
#                 )
#                 print(f"🔍 Optimized query: {optimized_query}")
#             except Exception as e:
#                 # Pokud optimalizace selže, použijeme původní dotaz
#                 print(f"⚠️ Query optimization failed: {e}")
#                 print(f"🔍 Using original query: {user_message}")
#                 optimized_query = user_message

#             # KROK 2: Vyhledání relevantních dokumentů ve vektorové databázi
#             # Použijeme optimalizovaný dotaz pro lepší přesnost vyhledávání
#             db_results = ...

#             # KROK 2.5: Kombinace dat z databáze A z internetu
#             # Vždy používáme oba zdroje pro kompletnější odpovědi
#             context_parts = []

#             print(f"🤖 Calling main AI model...")
#             try:
...
#                 print(f"✂️ Trimmed history to 20 messages")

#             # KROK 7: Odeslání odpovědi klientovi
#             print(f"📤 Sending response via WebSocket...")
#             try:
